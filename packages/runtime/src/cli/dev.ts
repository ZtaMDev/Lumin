...
import { loadConfig, findConfigPath, type LuminConfig, type RouteHeadConfig } from "./loader.js";
    ...
    for (const route of routes) {
      ...
      if (fs.existsSync(lumixPath)) {
        ...
        try {
          ...
          console.warn(`[lumix dev] Warning in ${route.path}: ${(error as Error).message}, using default 'prerender'`);
...
async function startSSGDevServer(cwd: string, config: any) {
  ...
  const lastErrors = new Map<string, { message: string; timestamp: number }>();
              ...
              if (file === configPath) {
                ...
                configChangeTimeout = setTimeout(async () => {
                  ...
                  try {
                    ...
                    console.error(pc.red(`  [lumix] Failed to restart server: ${(error as Error).message}\n`));
                  }
                }, 300);
              }
            });
...
<body>
  ...
  console.log(
    `  ${pc.green("➜")}  ${pc.bold("Local:")}   ${pc.cyan(`http://localhost:${port}/`)} ${pc.dim("(PIR/SSR Dev)")}`,
  );
  console.log(
    `  ${pc.dim("  PIR routes cached, SSR routes dynamic")}`,
  );
  console.log(
