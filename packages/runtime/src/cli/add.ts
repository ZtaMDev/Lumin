...
const INTEGRATIONS: Record<string, { deps: string[]; devDeps: string[]; run: (cwd: string) => Promise<void> }> = {
  tailwind: {
    deps: [],
...
export async function add(integration: string) {
  ...
  const toInstall = [...meta.deps, ...meta.devDeps].filter(
